...
const Admins = () => {
    ...
    const onPageChange = (page: number) => {
        ...
        get(`${import.meta.env.VITE_API_URL}admins?limit=${selectedOption}&page=${page}&filter=${selectedItem}&value=${translatedSearch}`);
        setCurrentPage(page);
    }
    ...
    const HandleDelteAdmin = async (id: number) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                del(`${import.meta.env.VITE_API_URL}admins/${id}`).then(() => {
                    setDeleted(!deleted);
                    Swal.fire(
                        t('Deleted!'),
    ...
    useEffect(() => {
        const translatedSearch = translateStatus(search, i18n.language);
        get(`${import.meta.env.VITE_API_URL}admins?limit=${selectedOption}&filter=${selectedItem}&value=${translatedSearch}`);
                ...
                <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
                    ...
                    <div className="py-6 px-4 md:px-6 xl:px-7.5 flex flex-col md:flex-row items-center justify-between">
                        ...
                        <div className="w-full  md:w-125 flex items-center justify-between relative border border-stroke dark:border-strokedark">
                            ...
                            <DropDownFilter items={["Id", "Username", "Role", "Status"]}
