...
const useAxios = () => {
  ...
  const fetchData = async ({ method, url, data = {}, config = {} }: { method: string, url: string, data?: any, config: any }) => {
    ...
    try {
      ...
      setError(err.response || 'Unexpected Error!');
    } finally {
      setLoading(false);
    }
  };
  ...
  const post = (url:string, data:any, config = {}) => fetchData({ method: 'post', url, data, config });
  const put = (url:string, data:any, config = {}) => fetchData({ method: 'put', url, data, config });
  const patch = (url:string, data:any, config = {}) => fetchData({ method: 'patch', url, data, config });
